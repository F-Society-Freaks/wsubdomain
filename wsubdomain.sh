...
###############################################################################
###############################################################################
### Modified: 09:27:36 - 2020-07-12

SITE="${1}"
if [[ -z ${SITE} ]]; then
     echo -e "internal error -- usage example: ./$basename$0 domain.nu"
     exit 1
fi

echo -e "--------------------------------------------"
echo -e "-> Searching subdomains for \e[4m$SITE\e[0m."
echo -e "--------------------------------------------"
curl -fsSL "https://crt.sh/?CN=%25.${SITE}&exclude=expired" \
    |grep -i ${SITE} \
    |sed 's/<TD>//;s/<\/TD>//g' \
    |awk '!seen[$0]++' \
    |grep "${1}$" \
    |sed -e 's/^[ \t]*//' \
    |awk '{print NR  "->  " $s}' > "./.wsubdomain"
if [[ "$(cat .wsubdomain|wc -l)"  -lt "1" ]]; then
     echo -e "-> $SITE has no certificate..."
     echo -e "--------------------------------------------\n"
     exit 1
else
     cat "./.wsubdomain"
     echo -e "--------------------------------------------"
     echo -e "-> Found $(cat .wsubdomain|wc -l) subdomain(s)"
     echo -e "--------------------------------------------"
     rm "./.wsubdomain"
fi
